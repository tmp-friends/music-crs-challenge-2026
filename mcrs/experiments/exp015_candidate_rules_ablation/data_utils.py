"""exp015 の dataset build で使う config / task helper。"""
...
def validate_config(config: Any) -> None:
    """exp015 の候補生成 config として安全な前提を検証する。

    Args:
        config: 候補生成 config。

    Raises:
        ValueError: retrieval_type または track_split_types が exp015 の前提と異なる場合。
    """
    if config.retrieval_type != "bm25":
        raise ValueError(f"exp015 expects retrieval_type='bm25', got {config.retrieval_type!r}")
    if list(config.track_split_types) != ["all_tracks"]:
        raise ValueError("exp015 dataset build must use track_split_types: ['all_tracks']")
