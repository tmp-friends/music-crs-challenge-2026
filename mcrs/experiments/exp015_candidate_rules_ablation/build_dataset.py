"""exp015 の LightGBM LTR 学習用 row dataset を構築する。"""
...
from mcrs.experiments.exp015_candidate_rules_ablation.data_utils import (
    config_list,
    prepare_tasks,
    validate_config,
)
from mcrs.experiments.exp015_candidate_rules_ablation.features import (
    LGBMFeatureBuilder,
)
...
def parse_sources(value: str | None, source_profile: str = "A") -> list[str]:
    """カンマ区切りの候補ソース名文字列を検証してリストに変換する。

    Args:
        value: カンマ区切りのソース名文字列。None または空文字列の場合は
            source_profile の default sources を返す。
    ...
    if value is None or not str(value).strip():
        return list(source_profile_to_sources(source_profile))
...
def new_candidate_metric_state() -> dict[str, Any]:
    ...
    return {
        ...
        "recall_sum_by_k": {20: 0.0, 100: 0.0, 500: 0.0},
...
def init_builders(
    ...
    *,
...
) -> tuple[Exp015CandidateBuilder, LGBMFeatureBuilder, set[str]]:
    ...
    Returns:
        (Exp015CandidateBuilder, LGBMFeatureBuilder, catalog_track_ids の集合) のタプル。
    """
    ...
    # CF/audio embedding DB は history source または embedding 特徴量が有効な場合のみ初期化する
    track_embedding_db = None
    user_embedding_db = None
    audio_embedding_db = None
    needs_history_embeddings = bool(enabled_set & set(HISTORY_SPLIT_SOURCES))
    if include_embedding_features or needs_history_embeddings:
    ...
    segment_popularity_index = (
        SegmentPopularityIndex(
            ...
            item_db=item_db,
            user_split_types=config_list(config, "user_split_types", ["all_users"]),
            split=config.get("segment_popularity_split", "train"),
        )
        if bool(enabled_set & set(SEGMENT_POPULARITY_SOURCES))
        else None
    )

    source_profile = str(config.get("source_profile", "A")).upper()
    wide_builder = Exp015CandidateBuilder(
        item_db=item_db,
        baseline_bm25=baseline_bm25,
        metadata_bm25=metadata_bm25,
        metadata_text_bm25=metadata_text_bm25,
        metadata_tag_bm25=metadata_tag_bm25,
        ...
        union_mode=config.get("union_mode", "quota" if source_profile == "G" else "rrf"),
        segment_popularity_index=segment_popularity_index,
    ...
    feature_builder = LGBMFeatureBuilder(
        ...
        source_names=wide_builder.source_names,
        source_weights={},
...
def build_split_rows(
    *,
    ...
    Args:
        config: 候補生成 config。
        wide_builder: 候補集合を構築する Exp015CandidateBuilder。
        ...
        candidate_topk: 各タスクで保持する候補の上限数。
    ...
    start = time.time()
    # feature_builder が source score を消費し、builder が score 付き retrieval を
    # 提供する場合のみ score を捕捉する。どちらか欠ければ従来経路（後方互換）。
    capture_scores = getattr(feature_builder, "consumes_source_scores", False) and hasattr(
        wide_builder, "batch_candidates_with_scores"
    )
    ...
    try:
        for batch_start in range(0, len(tasks), batch_size):
            ...
            for offset, task in enumerate(batch_tasks):
                ...
                labels = task["labels"]
                union_candidates = wide_builder.union(cbs, topk=candidate_topk)
                ...
                if labels and not has_positive:
                    if force_add_positive:
                        ...
                        if missing_labels:
                            keep = max(0, candidate_topk - len(missing_labels))
                ...
                group_rows = feature_builder.build_rows(
                    ...
                    labels=labels,
                    topk=candidate_topk,
                    scores_by_source=batch_scores[offset] if batch_scores is not None else None,
                )
...
def stabilize_parquet_dtypes(df: pd.DataFrame) -> pd.DataFrame:
    """Parquet streaming 用に欠損しやすい数値列の dtype を固定する。

    Args:
        df: 書き込み対象 batch の DataFrame。

    Returns:
        Parquet に渡すために dtype を整えた DataFrame。
    """
    df = df.copy()
    numeric_columns = [
        col
        for col in df.columns
        if str(col).startswith("f_") or col in {"candidate_rank", "label", "turn_number"}
    ]
    for col in numeric_columns:
        # 初回 batch が全欠損でも Arrow null 型に落ちないよう、特徴量は明示的に double へ寄せる。
...
class RowWriter:
    ...
    parquet では疎な特徴量列が batch ごとに Arrow null / double で揺れないよう dtype を固定する。
    """
    ...
    def write(self, rows: list[dict[str, Any]]) -> None:
        ...
        if self.path.endswith(".parquet"):
            df = stabilize_parquet_dtypes(df)
...
def main(args: argparse.Namespace) -> None:
    ...
    enabled_sources = parse_sources(
        args.enabled_sources,
        str(config.get("source_profile", "A")),
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build exp015 LightGBM LTR datasets.")
    ...
    parser.add_argument("--enabled_sources", default=None)
