...
.venv/bin/python mcrs/experiments/exp058_wide100_seed_ensemble/seed_ensemble_wide100.py \
  ...
  --source "cross_session:50=${OUT}/nopad/dev_tracks_cross_session.json" \
  ...
  --blind_source "cross_session:50=${OUT}/nopad/blindA_tracks_cross_session.json" \
  --exclude_json "${OUT}/within_session_exclude.json" \
  --allow_short_sources \
  --protect_top 0 \
  --num_boost_round 450 \
  ...
  --lambda_l2 8.0 \
  --num_threads 8 \
  --output_report "${OUT}/ranker/report_cross_fix_ens6.json" \
  --output_dev_full "${OUT}/ranker/dev_full_cross_fix_ens6.json" \
  --output_blind_tracks "${OUT}/ranker/blindA_tracks_cross_fix_ens6.json"
