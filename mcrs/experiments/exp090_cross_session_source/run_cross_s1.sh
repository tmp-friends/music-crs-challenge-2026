#!/usr/bin/env bash
# exp090 cross-session exact-track source の single-seed Dev OOF floor check。
# exp058 の 12 session-local source に cross-session source を 1 つ足して retrain（LGBM 再学習）し、
# Dev OOF が floor 0.177771（exp058 single-seed）を超えるかを見る。超えれば 6-seed ens + Blind A へ。
set -euo pipefail
...
.venv/bin/python mcrs/experiments/exp027_wide_source_lgbm/rank_source_lgbm_mixed_topk.py \
  ...
  --min_data_in_leaf 80 \
  --lambda_l2 8.0 \
  --output_report "${OUT}/ranker/s1_cross_report.json" \
  --output_dev_full "${OUT}/ranker/dev_full_s1_cross.json" \
  --output_dev_oof "${OUT}/ranker/dev_oof_s1_cross.json" \
  --output_blind_tracks "${OUT}/ranker/blindA_tracks_s1_cross.json"
