#!/usr/bin/env bash
# exp116 lxct only, symmetric 6-seed ensemble。lxct(lgbm+xgb+catboost+tabm)の 4 family すべてを
# 6 seed で seed-ensemble（full-fit / OOF とも 6-seed 対称）。user 指示で lxct のみ（lxc secondary 無し）。
...
.venv/bin/python mcrs/experiments/exp116_gbdt_family_ensemble/ensemble_rerank.py \
  ...
  --output_report "${OUT}/ranker/report_ens_lxct6.json" \
