#!/usr/bin/env bash
# exp116 symmetric 6-seed ensemble。4 family すべてを 6 seed で seed-ensemble（full-fit/OOF 対称）。
# 1 run で primary=lxct(lgbm+xgb+catboost+tabm) と secondary=lxc(lgbm+xgb+catboost) の両 blend を
# 同一 6-seed model から出力（CatBoost/TabM の二重学習を避け、両 zip の GBDT 部を完全一致させる）。
# 候補・特徴量・exclude は exp090-fix と byte-identical。
set -euo pipefail
...
.venv/bin/python mcrs/experiments/exp116_gbdt_family_ensemble/ensemble_rerank.py \
  ...
  --secondary_families "lgbm,xgb,catboost" \
  --seeds "20260616,20260617,20260618,20260619,20260620,20260621" \
  --xgb_n 6 --catboost_n 6 --tabm_n 6 \
  --oof_lgbm_n 6 --oof_xgb_n 6 --oof_catboost_n 6 --oof_tabm_n 6 \
  --xgb_device cuda --catboost_task_type GPU --tabm_device cuda --tabm_epochs 6 \
  ...
  --output_report "${OUT}/ranker/report_ens_sym6.json" \
  --output_dev_full "${OUT}/ranker/dev_full_ens_lxct6.json" \
  --output_blind_tracks "${OUT}/ranker/blindA_tracks_ens_lxct6.json" \
  --output_dev_full2 "${OUT}/ranker/dev_full_ens_lxc6.json" \
  --output_blind_tracks2 "${OUT}/ranker/blindA_tracks_ens_lxc6.json"
