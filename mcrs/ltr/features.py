"""LightGBM LTR の特徴量抽出 utility（共有ライブラリ）。

exp014 で実装した特徴量抽出を ``mcrs/ltr/`` へ切り出した共有モジュール。旧
``mcrs.experiments.exp014_lightgbm_ltr_reranker.features`` は re-export shim 経由で
このモジュールへ解決される。
"""
...
# text ベースの主要 5 ソース; build_dataset.py の --enabled_sources デフォルト値として使用される
            ...
            row.update(
                {
                    ...
                    "f_has_explicit_source_hit": float(track_id in source_rank_maps.get("exact_current_union", {})),
