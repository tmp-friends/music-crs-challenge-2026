...
def validate_eval_dataset_config(config, eval_dataset: str) -> None:
    expected_dataset_markers = {
        "blindset_a": "TalkPlayData-Challenge-Blind-A",
        "blindset_b": "TalkPlayData-Challenge-Blind-B",
    }
    normalized_eval_dataset = str(eval_dataset).strip().lower()
    expected_marker = expected_dataset_markers.get(normalized_eval_dataset)
    if expected_marker is None:
        return

    configured_dataset = str(config.test_dataset_name)
    if expected_marker not in configured_dataset:
        raise ValueError(
            f"--eval_dataset={eval_dataset} expects config.test_dataset_name "
            f"to contain '{expected_marker}', got '{configured_dataset}'."
        )
...
def main(args):
    ...
    validate_eval_dataset_config(config, args.eval_dataset)
